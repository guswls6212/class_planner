...
# 관측: production omni-radar 에러 모니터링 (radar.deepcraft.app, CF-gate). analytics 와 동일하게
# NEXT_PUBLIC 은 빌드타임 freeze → 여기 builder ARG/ENV 로 받아야 클라 번들에 baked.
ARG NEXT_PUBLIC_OMNI_RADAR_URL
ARG NEXT_PUBLIC_OMNI_RADAR_TOKEN
...
ENV NEXT_PUBLIC_OMNI_RADAR_URL=$NEXT_PUBLIC_OMNI_RADAR_URL
ENV NEXT_PUBLIC_OMNI_RADAR_TOKEN=$NEXT_PUBLIC_OMNI_RADAR_TOKEN
