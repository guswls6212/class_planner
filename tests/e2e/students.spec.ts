...
test.describe("Students 페이지 E2E 테스트", () => {
  ...
  test("새로운 학생을 추가할 수 있어야 한다", async ({ page }) => {
    // 학생 이름 입력
    await page.fill('input[placeholder*="학생 이름으로 검색"]', "김철수");
  ...
  test("Enter 키로 학생을 추가할 수 있어야 한다", async ({ page }) => {
    // 학생 이름 입력
    await page.fill('input[placeholder*="학생 이름으로 검색"]', "김영희");

    // Enter 키 누르기
    await page.press('input[placeholder*="학생 이름으로 검색"]', "Enter");
  ...
  test("학생 검색 기능이 작동해야 한다", async ({ page }) => {
    ...
    await page.fill('input[placeholder*="학생 이름으로 검색"]', "이민수");
    await page.click('button:has-text("추가")');
