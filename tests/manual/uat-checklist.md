...
**Steps:**
1. 시작시간 09:00, 종료 08:00 (역순 — `종료 < 시작`)
2. 다음 버튼 disabled 확인 + "종료 시간은 시작 시간보다 늦어야 합니다." 인라인 에러
3. 시작 11:00, 종료 22:00 (11시간 — **8시간 초과**)
4. 다음 버튼 disabled 확인 + "세션 시간은 최대 8시간까지 설정할 수 있습니다." 인라인 에러
5. 시작 14:00, 종료 16:00 (2시간 — 정상) → 에러 사라짐 + 다음 버튼 활성
**Expected:**
- picker 변경 시점에 즉시 에러 메시지 표시 (Step 1 시간 popover 내부 또는 chip 하단 inline)
- 8시간 초과·역순 모두 다음 버튼 disabled → Step 2 (확인) 진행 차단
- ⚠️ PR #416 이전 버그: 8시간 초과해도 다음 버튼 활성 → Step 2 진행 → 수업 추가 클릭 무반응 → 이전 버튼으로 돌아가야 에러 보임 (silent fail). picker 변경 시점에서 막혀야 정상.
**Result:** [ ] Pass [ ] Fail — note: ___
