...
2. Project URL, anon public, service_role 키 복사

### 프로덕션 환경 (.env.production)

Lightsail 서버에서 직접 생성 (Git 커밋 금지):
```bash
...
EOF
```
Docker Compose에서 `env_file: .env.production`으로 자동 주입.

### Next.js 환경 변수 규칙

- `NEXT_PUBLIC_` 접두사: 클라이언트 사이드 접근 가능 (브라우저 노출)
- 접두사 없음: 서버 사이드 전용 (API Routes에서만 접근)

### 보안 주의사항

1. `SUPABASE_SERVICE_ROLE_KEY`는 절대 프론트엔드에 노출 금지
2. `.env.local`, `.env.production`은 Git 커밋 금지
3. CI/CD에서는 GitHub Secrets로 관리
