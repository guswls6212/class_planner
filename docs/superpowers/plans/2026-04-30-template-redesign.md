...
### Task 0.3: 신규 학생 자동 생성 ✅ 옵션 (a) 확정

매칭 실패한 학생 이름은 **자동 생성** + 토스트로 "신규 학생 N명 자동 추가됨" 알림.

⚠️ 부작용 수용: 오타로 인한 매칭 실패도 자동 생성됨 (예: "김 철수"가 별도 학생). 5명 사용자 규모에서 학생 페이지에서 즉시 정리 가능.
    ...
    sessions: currentWeekSessions.map(s => {
      ...
      const studentNames = s.enrollmentIds
        .map(eid => {
          ...
          return enr ? students.find(st => st.id === enr.studentId)?.name : null;
        })
...
const doApplyTemplate = async (template: TemplateRow) => {
  ...
  try {
    ...
    // 2. 템플릿 세션들 생성 (학생 매칭 + 신규 생성)
    let applied = 0;
    let createdStudents = 0;
    for (const tplSession of template.template_data.sessions) {
      // subject 찾거나 생성
      let subjectId = subjects.find(s => s.name === tplSession.subjectName)?.id;
      if (!subjectId) {
        const newSubject = { id: crypto.randomUUID(), name: tplSession.subjectName, color: tplSession.subjectColor };
        await updateData({ subjects: [...subjects, newSubject] });
        subjectId = newSubject.id;
      }

      // students 찾거나 생성 (Phase 0 Task 0.3 = 옵션 a 가정)
      const studentIds: string[] = [];
      for (const studentName of tplSession.studentNames) {
        let st = students.find(stu => stu.name === studentName);
        if (!st) {
          st = { id: crypto.randomUUID(), name: studentName };
          await addStudent(st);
          createdStudents++;
        }
      ...
      // enrollment 매핑
      const enrollmentIds = studentIds.map(stId => {
        let enr = enrollments.find(e => e.studentId === stId && e.subjectId === subjectId);
        if (!enr) {
          enr = { id: crypto.randomUUID(), studentId: stId, subjectId: subjectId! };
          // updateData에서 enrollments 추가 (실제 코드는 useEnrollmentsLocal 등에 위임)
        }
        return enr.id;
      });
    ...
    toast.success(
      `${applied}개 수업이 템플릿으로 교체되었습니다${createdStudents > 0 ? ` (학생 ${createdStudents}명 자동 추가)` : ""}.`
    );
  } catch (e) {
