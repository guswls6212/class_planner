...
- [ ] 학생 이름 입력창에 텍스트 입력 가능
- [ ] 빈 이름 입력 시 추가 버튼 비활성화
- [ ] 중복 이름 입력 시 경고 메시지 표시
- [ ] 유효한 이름 입력 시 학생 목록에 추가됨
- [ ] 추가 후 입력창 초기화
...
- [ ] 7일(월~일) x 시간별(9:00~24:00) 그리드 표시
- [ ] 요일별 헤더(월, 화, 수, 목, 금, 토, 일) 표시
- [ ] 시간별 헤더(9:00, 10:00, 11:00...) 표시
- [ ] 세션이 없는 시간대는 빈 셀로 표시

### ✅ **세션 표시**

- [ ] 등록된 수업 세션이 해당 시간대에 표시됨
- [ ] 세션 블록에 과목명과 학생명 표시
- [ ] 과목별 색상으로 구분 표시
- [ ] 겹치는 세션들이 Y축으로 분리되어 표시
- [ ] 세션 블록에 요일 표시 (예: "월 국어 09:00-10:00")

### ✅ **학생별 필터링**

- [ ] 학생 선택 시 해당 학생의 수업만 표시
- [ ] 학생 선택 해제 시 전체 학생의 수업 표시
- [ ] 필터링 상태가 localStorage에 저장됨
...
- [ ] 학생 목록이 localStorage에 저장됨
- [ ] 과목 목록이 localStorage에 저장됨
- [ ] 수강신청 정보가 localStorage에 저장됨
- [ ] 수업 세션이 localStorage에 저장됨
- [ ] UI 상태(선택된 학생, 패널 위치)가 localStorage에 저장됨
