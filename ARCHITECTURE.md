...
- **Molecules:** Atoms 조합
  ...
  - `AdminInviteRow`, `ActiveAdminMemberRow` (settings/page.tsx inline) — 관리자 초대 대기 / 가입 완료 관리자 row. teacher 미연동 admin 의 추적/제외 UX (PR #445 + #455).
...
- 2026-05-23: 강사·멤버 관리 통합 + invite UX 재설계 (PR #444-#458, 15개). **DB migrations**: 045 `invite_tokens.invitee_label`, 046 `teachers.archived_at` + partial index. **API**: `POST /api/invites/[id]/regenerate` (atomic token rotation), `POST /api/teachers/[id]/archive` (archived_at 토글), `POST /api/teachers/[id]/reassign` (sessions teacher_id 일괄 이전 + 원 강사 자동 보관), `DELETE /api/members/[userId]` 권한 확장 (owner=모두 / admin=member만 / teachers.user_id 복원), `GET /api/invites` 응답 label, `GET /api/teachers?includeArchived`. **Molecules**: `TypedConfirmationModal` (위험 액션 typing 확인 패턴, GitHub repo delete 풍), `ReassignTeacherModal` (교체 옵션 + 영향 미리보기). **Settings UX (mockup team-invite-redesign Variant C + Option 1)**: admin invite 별칭 입력 + admin invite row (원장 다음, 강사 위), 가입 admin 멤버 row (`ActiveAdminMemberRow`, teacher 미연동 admin 표시), 메뉴 라벨 정리 ("재발송"→"새 링크 발급"), `getMenuItems` 정리 (링크 복사 메뉴 제거 — quick action 중복 해소). **Teacher lifecycle (mockup teacher-replace-ux Variant C/B)**: archive 패턴 도입 — '삭제' 라벨 → '보관' (hard DELETE 회피, 데이터 보존), 강사 페이지 헤더 "보관된 강사 보기" toggle + 1-click 복구, "다른 강사로 교체" 메뉴 항목 + Modal (대체 강사 라디오 + archiveOriginal 체크박스). **Bug fixes**: invite OAuth callback redirect 가드 (PR #454 — `useGlobalDataInitialization` pathname 가드에 `/invite/*` 추가), invite accept 직후 빈 schedule race fix (PR #456 — `setActiveAcademyId` 사전 호출 + `class-planner:academy-changed` event dispatch), settings stale localStorage sync fix (PR #448 — `deleteTeacherFromLocal` 추가 호출), UAT cleanup audit_log 누락 (PR #457 — FK RESTRICT cleanup 보강), schedule 첫 진입 토스트 false-positive (PR #458 — `useScheduleMeta` fetchMeta 내 lastViewed=NULL 분기 즉시 ack). **Design exploration mockups**: 3개 신설 (team-invite-redesign / member-removal-ux / teacher-replace-ux) — Phased 도입 결정 (Archive Phase 1 → Reassign Phase 2 → 완전 삭제 Phase 3 deferred). **/login UI**: 카카오 disabled 메뉴 (준비 중), "로그인" h2 제거 (헤더 중복).
