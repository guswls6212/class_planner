...
describe("TimeTableRow Component", () => {
  ...
  describe("drop-preview ghost", () => {
    ...
    const draggedSession = makeSession("dragged", 1, "09:00", "10:00");

    it("dragPreview가 일치하는 weekday+time이면 ghost가 렌더된다", () => {
      const sessions = new Map<number, Session[]>();
      sessions.set(0, [draggedSession]);
      ...
      expect(screen.getByTestId("drop-preview-ghost")).toBeInTheDocument();
    });

    it("targetWeekday가 다르면 ghost가 렌더되지 않는다", () => {
      const sessions = new Map<number, Session[]>();
      sessions.set(0, [draggedSession]);
      ...
      render(
        <TimeTableRow
          ...
          dragPreview={{
            draggedSession,
            targetWeekday: 1, // 다른 요일
            targetTime: "10:00",
            targetYPosition: 0,
          }}
        />
      );
      expect(screen.queryByTestId("drop-preview-ghost")).not.toBeInTheDocument();
    });

    it("dragPreview가 없으면 ghost가 렌더되지 않는다", () => {
      const sessions = new Map<number, Session[]>();
      sessions.set(0, [draggedSession]);
